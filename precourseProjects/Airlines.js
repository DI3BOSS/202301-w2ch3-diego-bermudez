...
const userWelcome = () => {
  ...
  userData.name = prompt(
    `${greetingType}, Por favor introduzca su nombre de usuario`
  );
  ...
  if (!userData.name || !isNaN(userData.name)) {
    ...
    alert(
      `${userData.name}, ISDI AIRLINES le da bienvenida. A continuación verá los vuelos disponibles para hoy`
    );
  }
};
...
--------------------------------- ISDI AIRLINES: VUELOS DEL DÍA ---------------------------------
  ...
  line.forEach((flight) => {
    console.log(
      `Vuelo ISDI #00${flight.id}, ${
        flight.scale ? "con" : "sin"
      } escalas. Origen: ${flight.from} | Destino: ${
        flight.to
      }. Plazas disponibles desde ${flight.cost}€`
    );
  });
...
const moreInfoNeeded = () => {
  userData["Interested in more info"] = confirm(
    `${userData.name}, ¿desea ver más información?`
  );
...
const moreFlightInfo = () => {
  ...
  averageFlightCost =
    (Math.round(averageFlightCost.reduce((a, b) => a + b) / flights.length) *
      100) /
    100;
...
const showScaledFlightList = (scale) => {
  scale.forEach((flightLine) => {
    console.log(
      ` > Vuelo ISDI #00${flightLine.id}, Origen: ${flightLine.from} | Destino: ${flightLine.to}`
    );
  });
...
[ÚLTIMOS 5 DESTINOS DEL DÍA]: Estos son nuestros últimos destinos del día con plazas disponibles:`);
  ...
  destination.forEach((destinationLine) => {
    console.log(
      ` > Vuelo ISDI #00${destinationLine.id}, DESTINO: ${destinationLine.to} (con origen en ${destinationLine.from})`
    );
  });
