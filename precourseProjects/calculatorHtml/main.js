  ...
  operator.addEventListener("click", () => {
    ...
    if (equalOperatorCounter === 1 || id !== "equal") {
      ...
      if (!calculatorOperations.operator) {
        ...
        calculatorOperations.numberA = Number(inputsBuffer[0]);
      } else {
        calculatorOperations.numberB = Number(inputsBuffer[1]);
        inputsBuffer.shift();
...
const equalStopper = () => {
  ...
  if (
    !isNaN(
      Number(displayData.substring(displayData.length - 1, displayData.length))
    )
  ) {
