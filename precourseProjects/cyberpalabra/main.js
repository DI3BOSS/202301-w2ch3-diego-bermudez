...
const commentsToGuessedAnswers = (random) => {
  const answers = [
    `¡Si Señor! Sigue así, ${playerData.name}, ¡a por la siguiente! ;)`,
    "Joder, esa no me la sabía, te fue bien la cogorza :D",
    `¡bién, Más eddies pa'la saca!. Tira pa'alante, ${playerData.name}`,
    "Es que si fallas, te dejo de hablar ^_^. ¡Venga! a por otra",
    `Ahí picando hielo! ${playerData.name}, #TheIceBreaker!.`,
...
const commentsToWrongAnswers = (random) => {
  const answers = [
    `¿Que coño te pasa, ${playerData.name}?, no me falles la siguiente! :(`,
    "¿En serio no sabías eso? ¡Espabila, que quiero salir de pobre!",
    "Te emborrachas, pierdes el pincho y ahora me la lias con algo tan básico... u_u",
    `¿Que sabías romper el hielo de Arasaka, decías?... Venga ${playerData.name}, véndele la moto a otro ¬_¬`,
    `${playerData.name}, piensa antes de escribir. ¡Hazlo por mi corazón, que me va a dar algo! :s`,
    "Si no fuera porque estás en peligro de muerte, diría que te la suda fallar",
  ];
  return answers[random];
};
...
const commentsToPassedAnswers = (random, id) => {
  const answers = [
    ...
    "¿por qué has pasado? No era tan difícil... No, no te la chivo xD",
    "¡De verdad, que ganas de postergar lo inevitable!",
    `${playerData.name}, ¿te gusta guardar lo más fácil para el final?`,
    `¡${questionPacks[id].answer[playerData.questionsIdForGame]}, ${
      questionPacks[id].answer[playerData.questionsIdForGame]
    }! Recuérdalo pa' la vuelta ;)`,
    "Bueno, va... al menos tendrás tiempo pa' pensarla",
  ];
  return answers[random];
};
...
const screenWidthAdapter = () => {
  switch (screen.width) {
    ...
    default:
      alert(
        `Su ventana mide ${screen.width} x ${screen.height}. Este juego está pensado para monitores Full HD. Por favor, ajuste manualemente el zoom para asegurarse de ver cómodamente todo el contenido de la pantalla de juego. ¡Gracias!`
      );
      break;
  }
};
...
const startGameMenu = () => {
  ...
  gameScenario.innerHTML +=
    ...
    playerData.name +
    ", da la señal y empezamos a invadir.</p>" +
...
const answerInputEvaluator = (id) => {
  ...
  playerInput.addEventListener("click", (event) => {
    ...
    if (
      ...
      setTimeout(() => {
        currentQuestionDisplayReset.innerHTML = `¡Sí, ${
          questionPacks[id].answer[playerData.questionsIdForGame]
        }!`;
      ...
      if (
        playerAnswer.value.toLowerCase() === "(ツ)_/¯ lúcete aquí..." ||
        playerAnswer.value.toLowerCase() === ""
      ) {
        setTimeout(() => {
          playerFailedSound.play();
          currentQuestionDisplayReset.innerHTML = `¡¿Por qué le das sin escribir?!. Era ${
            questionPacks[id].answer[playerData.questionsIdForGame]
          }`;
        }, 100);
      } else {
