...
const questionidCounter = 0;
console.log(
  questionPacks[questionidCounter].question[playerData.questionsIdForGame]
);
...
const timer = () => {
  timeInterval.finish = new Date(
    timeInterval.start.getTime() + gameTime * 1000
  );

  const secondsLeft = Math.ceil(
    (timeInterval.finish.getTime() - new Date().getTime()) / 1000
  );
  if (secondsLeft <= 0) {
    return 0;
  }
...
const gameStart = () => {
  const gameRunner = new Promise(
    (opt = (keepPlaying, finishGame) => {
      questionPacks.forEach((letter) => {
        letter.status = prompt(
          letter.question[playerData.questionsIdForGame],
          playerData.name === "admin"
            ? letter.answer[playerData.questionsIdForGame]
            : ""
        );
...
`);
 ...
 ${
   playerData.name === "admin"
     ? "🚨 ¡OYEEEE! ¡has usado el truco del admin!, acuerdate del trato ;)"
     : ""
 }
...
▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁ `);
    ...
    ✔ ${
      playerData.points < 1
        ? "No has tenido ningún acierto."
        : "has acertado " + playerData.points + " palabras"
    }. En pantalla verás el ranking.
▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁ `);
...
const otherRound = () => {
  alert(`▁ ▁ ▂ ▃ ▄ ▅ ▆ ✪  P A S A P A L A B R A  ✪ ▆ ▅ ▄ ▃ ▂ ▁ ▁
                 ...
                 ✔ ${
                   playerData.points < 1
                     ? "No has tenido ningún acierto aún."
                     : "Hasta ahora, has acertado " +
                       playerData.points +
                       " palabras."
                 }
▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁ `);

  const gameRound2 = new Promise(
    (opt = (keepPlaying, finishGame) => {
      questionPacks.forEach((letter) => {
        if (letter.status === "🔵") {
          letter.status = prompt(
