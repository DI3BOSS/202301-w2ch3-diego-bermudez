...
let takenBalls = []
    ...
    const randomBallNumbers = () => {
        return Math.ceil(Math.random() * ballsInGame.length);
    };
...
const bingoCardNumbersGenerator = () => {
    ...
    do {
        let randomBuffer = randomBallNumbers();
...
const bingoCardGenerator = () => {
    ...
    let rowOne = bingoCardNumbers.slice(0, 5)
    let rowTwo = bingoCardNumbers.slice(5, 10)
    let rowThree = bingoCardNumbers.slice(10)
...
console.log(`
    ...
    ╠═════════════════════════════════╣
             L1       L2       L3
        B   【${twoDigitsforCard(bingoCard[0][0])}】   【${twoDigitsforCard(bingoCard[1][0])}】   【${twoDigitsforCard(bingoCard[2][0])}】
    ║                                 ║
        I   【${twoDigitsforCard(bingoCard[0][1])}】   【${twoDigitsforCard(bingoCard[1][1])}】   【${twoDigitsforCard(bingoCard[2][1])}】
    ║                                 ║
        N   【${twoDigitsforCard(bingoCard[0][2])}】   【${twoDigitsforCard(bingoCard[1][2])}】   【${twoDigitsforCard(bingoCard[2][2])}】
    ║                                 ║
        G   【${twoDigitsforCard(bingoCard[0][3])}】   【${twoDigitsforCard(bingoCard[1][3])}】   【${twoDigitsforCard(bingoCard[2][3])}】
    ║                                 ║
        O   【${twoDigitsforCard(bingoCard[0][4])}】   【${twoDigitsforCard(bingoCard[1][4])}】   【${twoDigitsforCard(bingoCard[2][4])}】

    ╚═════════════════════════════════╝
`);

    playerData['boleto aceptado'] = confirm(`Tienes en pantalla tu cartón de juego.
...
const drumNumberBallTaker = () => {
    ...
    } else if ( bingoCard[0].completed && bingoCard[1].completed&& bingoCard[2].completed ) {
...
▁▁▁▁▁ ▁ ▂ ▃ ▄ ▅ ▆ ✪ B I N G O ✪ ▆ ▅ ▄ ▃ ▂ ▁ ▁▁▁▁▁
        ...
        Ⓟ PUNTOS RESTANTES: ${playerData.points + PlayerPointsSubstraction} de ${maximumPlayerPoints}.
        ▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁
    ...
    __   __  _____  _     _      _  _  _ _____ __   _  |
      ╲_/   |     | |     |      |  |  |   |   | ╲  |  |
       |    |_____| |_____|      |__|__| __|__ |  ╲_|  ·
        ...
        pointsRanking.push({player: playerData.name, ['points left']: playerData.points}),
        console.table(pointsRanking.sort((a, b) => (b['points left'] - a['points left'])));
    ...
    _______ _     _ _______ __   _ _     _ _______      _______  _____   ______       _____         _______ __   __ _____ __   _  ______  |
     ...
     let numberBallTaker = randomBallNumbers();

        if (takenBalls.findIndex(ball => ball === numberBallTaker) === -1) {

            takenBalls.push(numberBallTaker);

                const ColumOneChecker = Object.values(bingoCard[0]).findIndex(number => number === numberBallTaker);
                ...
                const ColumTwoChecker = Object.values(bingoCard[1]).findIndex(number => number === numberBallTaker);
                ...
                const ColumThreeChecker = Object.values(bingoCard[2]).findIndex(number => number === numberBallTaker);
    ...
    const numberBallTaken = confirm (`    Ⓡ RONDA: ${takenBalls.length} de ${ballsInGame.length}    ║    Ⓟ PUNTOS RESTANTES: ${playerData.points} de ${maximumPlayerPoints}.
    ▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁ 
                      SALE DEL BOMBO LA BOLA NÚMERO...
    
                                        ✪ 【${twoDigitsforCard(numberBallTaker)}】 ✪
    ▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁ 
